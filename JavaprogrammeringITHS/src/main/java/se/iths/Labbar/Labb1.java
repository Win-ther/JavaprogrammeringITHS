...
public class Labb1 {
    ...
    public static void visualisering(int[] price) {
        ...
        step = Math.ceil(step);
        //If/Else satsen är bara där för det andra testet createDiagram2, den har lite ändrade värden för vart x ska placeras ut för att jag inte kunde få utskriften att se exakt likadan ut som i det testet
        //  går att ta bort och inte användas ifall man inte skulle använda det andra testet.
        if (min >= 0) {
            for (int i = 0; i < 6; i++) {
                switch (i) {
                    case 0 -> {
